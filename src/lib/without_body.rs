...
/// the request BODY
///
pub fn forward_request_without_body(incoming_request: &HttpRequest<ProxyOpts>, mut outgoing: ClientRequestBuilder) -> Box<Future<Item=HttpResponse, Error=Error>> {
    ...
    outgoing.finish().unwrap().send().map_err(Error::from)
        .and_then(move |proxy_response| {
            ...
            if should_rewrite_body(proxy_response.headers()) {
                ...
                Either::B(pass_through_response(proxy_response, req_uri, target_domain))
            }
        })
        .responder()
}
...
fn pass_through_response(proxy_response: ClientResponse, req_uri: Uri, target_domain: String) -> Box<Future<Item=HttpResponse, Error=Error>> {
    ...
    let output = ok(
        create_outgoing(&proxy_response.headers(), target_domain.to_string(), req_target)
            .body(Body::Streaming(Box::new(proxy_response.payload().from_err())))
    );
...
///
fn response_from_rewrite(proxy_response: ClientResponse, req_uri: Uri, target_domain: String) -> Box<Future<Item=HttpResponse, Error=Error>> {
    let next_host = req_uri.clone();

    let output = proxy_response.body()
        .from_err()
        .and_then(move |body| {
            use std::str;
